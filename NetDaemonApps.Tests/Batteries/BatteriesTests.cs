...
public class BatteriesTests
{
    ...
    public void NotifiesWhenBatteriesAreFlat()
    {
        ...
        ctx.VerifyCallService("notify.twinstead", Times.Once, new { Message = "Radiator batteries are flat" });
    }

    [Test]
    public void NotifiesWhenBatteriesAreAlmostFlat()
    {
        var ctx = new AppTestContext();
        ctx.Scheduler.AdvanceTo(new DateTime(2022, 11, 07, 6, 59, 59).Ticks);
        var entrance = ctx.GetEntity<NumericSensorEntity>("sensor.wiser_itrv_entrance_battery", "100");
        ...
        var app = ctx.InitBatteries();
        ctx.Scheduler.AdvanceBy(TimeSpan.FromSeconds(1).Ticks);
        ctx.VerifyCallService("notify.twinstead", Times.Once, new { Message = "Radiator batteries are almost flat" });
    }
...
{
    public static BatteriesApp InitBatteries(this AppTestContext ctx) => new(ctx.HaContext, ctx.Scheduler, new Mock<ILogger<BatteriesApp>>().Object);
}